...
<Meta title="Welcome/Guides/React Style Guide" />
...
This guide is created based off the styling of Airbnb. Please read below to get more information on the styling guide.
...
1. [Never Nest Hooks](#never-nest-hooks)
1. [Naming](#naming)
1. [Ternary](#ternary)
...
1. [Implicit return](#implicit-return)
...
Since React components are re-rendered each time the data changes, this means the exact same hooks
must be called in the exact same order on every single render. If we wrapped our hooks in a conditional or
function, the state would sometimes be created and other times wouldn’t.
...
- Do not use `if` and `else` for conditional statements. Instead use a ternary operator.
...
- Ideal for options greater than two where ternary operator cannot apply.
...
- Always use Fragment over Div.
  ...
  > Why? For performance because one less node is created in the virtual DOM.
...
- CSS Style Guid - https://www.smashingmagazine.com/2008/08/7-principles-of-clean-and-optimized-css-code/
